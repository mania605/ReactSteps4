...
//fetchFn
const fetchFlickr =({queryKey}) => {
  //console.log(opt);//먼저 opt값 잘 전달되는지 확인
...
//useQuery를 활용한 실체 호출할 커스텀훅 등록
export const function useFlickrQuery= opt =>{
  return useQuery({
    queryKey : ['flickrQuery',opt],
    queryFn: fetchFlickr,
    staleTime: 1000* 60,
    gcTime: 1000 * 60   // 캐시타임이라고 하는 회사도 있음
  });
};
